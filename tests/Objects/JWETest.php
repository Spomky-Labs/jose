...
class JWETest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $jwe = new JWE();
        $jwe = $jwe->withProtectedHeader([
            'jty'  => 'JWT',
            'cty'  => 'JOSE+JSON',
            'crit' => ['alg', 'iss'],
            'zip'  => 'DEF',
        ]);
        $jwe = $jwe->withProtectedHeaderValue('foo', 'www.example.com');
        $jwe = $jwe->withUnprotectedHeader([
            'alg' => 'ES256',
            'enc' => 'A128CBC-HS256',
        ]);
        $jwe = $jwe->withUnprotectedHeaderValue('bar', 'me@example.com');
        $jwe = $jwe->withPayload([
            'jti' => 'ABCD',
            'iss' => 'me.example.com',
            'aud' => 'you.example.com',
