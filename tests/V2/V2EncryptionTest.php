...
class V2EncryptionTest extends \PHPUnit_Framework_TestCase
{
    public function testCreateJWEAndEncrypt()
    {
        ...
        $jwe = $jwe->withSharedHeaders(['foo' => 'bar']);
        $encrypter = EncrypterFactory::createEncrypter(['A256CBC-HS512', 'RSA-OAEP-256']);
        $public_key = $this->getRSAPublicKey();
        $private_key = $this->getRSAPrivateKey();

        $jwe = $encrypter->addRecipient($jwe, $public_key);

        $compact = $jwe->toJSON();

        $loaded = Loader::load($compact);

        $decrypter = DecrypterFactory::createDecrypter(['alg' => 'RSA-OAEP-256', 'enc' => 'A256CBC-HS512']);

        $result = $decrypter->decryptUsingKey($loaded, $private_key);
    }
