...
class ECDH_ES_A128KWAndA128GCMEncryptionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testECDH_ES_A128KWAndA128GCMEncryption()
    {
        ...
        $add = [];
        $alg = new ECDHESA128KW();
        $adf = $alg->wrapAgreementKey(
            $ephemeral_key,
            $private_key->toPublic(),
            Base64Url::decode('Nou2ueKlP70ZXDbq9UrRwg'),
            (new A128GCM())->getCEKSize(),
            $protected_headers,
            $add
        );
        var_dump($add);
        var_dump($adf);
        var_dump(Base64Url::encode($adf));*/
